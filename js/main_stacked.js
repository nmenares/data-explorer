...
Promise.all([
  ...
  function filterData(){
    ...
    uniqueGroupBy.forEach(d => {
      ...
      years.forEach((y, i) => {
        ...
        obj.key = d;
        obj.date = dateParse(y);
        obj.value = thisGroup.reduce((a,b) => a + b[y], 0);
        state.dataToPlot.push(obj);
      });
    })
    console.log(state.dataToPlot)

    let keys = uniqueGroupBy;
    let values = Array.from(d3.rollup(state.dataToPlot, ([d]) => d.value, d => +d.date, d => d.key));
    ...
    state.dataToPlot = series;

    // console.log(d3.rollup(state.filteredData, ([d]) => d.value, d => +d.date, d => d.key))

    // console.log(d3.stack().keys(uniqueGroupBy)(state.filteredData))
    // uniqueGroupBy.forEach(d => {
    //   let obj = {};
    //   obj.name = d;
    //   let thisGroup = state.filteredData.filter(s => s[state.groupBy] === d);
  ...
  function updateAxes() {
    ...
    if (state.scale === "log"){
      ...
      let yMax = d3.max(state.dataToPlot, d => d3.max(d, d => d[1])),
          yMin = d3.min(state.dataToPlot, d => d3.min(d, d => d[1]));
      // let yMax = 1.0;
